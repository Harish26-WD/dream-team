...
from app.models import Department, Employee, Designation
...
class TestModels(TestBase):
    ...
    def test_designation_model(self):
        """
        Test number of records in Designation table
        """

        # create test designation
        designation = Designation(name="CEO", description="Run the whole company")

        # save designation to database
        db.session.add(designation)
        db.session.commit()

        self.assertEqual(Designation.query.count(), 1)
...
class TestViews(TestBase):
    ...
    def test_designations_view(self):
        """
        Test that designations page is inaccessible without login
        and redirects to login page then to designations page
        """
        target_url = url_for('admin.list_designations')
        redirect_url = url_for('auth.login', next=target_url)
        response = self.client.get(target_url)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, redirect_url)
